 ...
 * @returns 过滤后的数据数组
 */
export const filterDataByDate = (data: any[], dateStr: string): any[] => {
 ...
 * @returns 24小时的空数据数组
 */
export const createEmptyHourlyData = (dateStr: string): any[] => {
 ...
 * @returns 热力图数据数组
 */
export const generateHeatmapFromTasks = (tasks: any[]): any[] => {
  ...
  // 将映射转换为热力图数据格式
  const heatmapData: {date: string, count: number, value: number}[] = [];
 ...
 */
export const processContentStats = (contentStats: any[]): any[] => {
  ...
  // 过滤掉duration为0的项
  return processedStats.filter((item: any) => item.duration > 0);
};
