...
export class PlanStorageService {
    ...
    async getAllPlans() {
        try {
            ...
            // 获取本地计划
            const localPlans = storage.getItem(PLANS_STORAGE_KEY, []);
            ...
            for (const localPlan of localPlans) {
                // 检查是否已存在于API计划中
                const exists = apiPlans.some((apiPlan) => this.arePlansEqual(apiPlan, localPlan));
                if (!exists) {
            ...
            // 如果出错，返回本地计划
            return storage.getItem(PLANS_STORAGE_KEY, []);
        }
    }
    /**
     * 添加新计划
    ...
    async addPlan(plan) {
        try {
            ...
            if (apiSuccess && apiPlan) {
                ...
                // 获取现有计划并添加新计划
                const plans = storage.getItem(PLANS_STORAGE_KEY, []);
     ...
     * 同步本地计划到API
     */
    async syncLocalPlansToAPI(localPlans, apiPlans) {
        // 获取需要同步的计划（本地有但API没有的）
        const plansToSync = localPlans.filter(localPlan => !apiPlans.some(apiPlan => this.arePlansEqual(apiPlan, localPlan)));
        ...
        // 同步每个计划
        const syncResults = await Promise.allSettled(plansToSync.map(plan => {
            // 创建一个新的计划对象，确保格式正确
            const apiPlan = {
     ...
     * 检查两个计划是否相同
     */
    arePlansEqual(plan1, plan2) {
        // 如果有ID且相同，则认为是同一个计划
        if (plan1.id && plan2.id && plan1.id === plan2.id) {
            return true;
        }
        // 否则比较内容
        return ((plan1.title === plan2.title || plan1.text === plan2.title || plan1.title === plan2.text) &&
