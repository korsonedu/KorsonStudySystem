 ...
 * @returns 图表数据
 */
export const generateChartData = (currentView, dailyStats, weeklyStats, monthlyStats, timeDistributionData) => {
    let labels = [];
    let data = [];
    try {
        if (currentView === 'daily') {
            ...
            if (Array.isArray(weeklyStats) && weeklyStats.length > 0) {
                // 直接使用weeklyStats中的数据
                labels = weeklyStats.map((item) => item.day || item.date || '');
                data = weeklyStats.map((item) => item.duration || item.value || 0);
            }
            else {
                // 尝试从任务数据生成周统计
            ...
            if (Array.isArray(monthlyStats) && monthlyStats.length > 0) {
                ...
                labels = monthlyStats.map((item) => item.day || item.date || '');
                data = monthlyStats.map((item) => item.duration || item.value || 0);
            }
            else {
                // 尝试从任务数据生成月统计
                if (Array.isArray(dailyStats) && dailyStats.length > 0) {
 ...
 * @returns 内容图表数据
 */
export const generateContentChartData = (currentView, dailyContentStats, weeklyContentStats, monthlyContentStats) => {
    let contentStats = [];
    const today = new Date();
    ...
    try {
        if (currentView === 'daily') {
            ...
            if (Array.isArray(dailyContentStats) && dailyContentStats.length > 0) {
                ...
                dailyContentStats.forEach((item) => {
                    // 尝试获取内容名称
