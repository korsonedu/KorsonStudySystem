...
export interface StatisticsData {
  ...
  // 内容统计数据
  dailyContentStats: Array<{
    name: string;
    duration: number;
  }>;

  // 每周内容统计数据
  weeklyContentStats: Array<{
    name: string;
    duration: number;
  }>;

  // 每月内容统计数据
  monthlyContentStats: Array<{
    name: string;
    duration: number;
  }>;
}
 ...
 * 获取基本统计数据
 */
export const fetchBasicStats = async (): Promise<{
  dailyDuration: number;
  weeklyDuration: number;
  monthlyDuration: number;
  totalHours: number;
    ...
    if (weeklyDuration === undefined || weeklyDuration === null) {
      try {
        ...
        if (tasks.length > 0) {
          ...
          // 计算本周任务总时长
          weeklyDuration = tasks.reduce((total: number, task: any) => {
            try {
              ...
              if (isNaN(taskDate.getTime())) return total; // 无效日期
    ...
    if (monthlyDuration === undefined || monthlyDuration === null) {
      try {
        ...
        if (tasks.length > 0) {
          ...
          // 计算本月任务总时长
          monthlyDuration = tasks.reduce((total: number, task: any) => {
            try {
              const startTime = task.start || task.start_time || task.startTime || task.date || '';
              if (!startTime) return total;
 ...
 * 获取每日统计数据
 */
export const fetchDailyStats = async (): Promise<Array<{
  hour: number;
  time: string;
  duration: number;
}>> => {
  try {
    ...
    if (response.data?.hourly && Array.isArray(response.data.hourly)) {
      return response.data.hourly.map((item: any) => ({
 ...
 * 获取每周统计数据
 */
export const fetchWeeklyStats = async (): Promise<Array<{
  day: string;
  duration: number;
}>> => {
  try {
    ...
    if (Array.isArray(response.data)) {
      ...
      // 处理响应数据
      response.data.weekly.forEach((item: any) => {
        const day = item.day || '';
        const duration = item.duration || 0;
 ...
 * 获取每月统计数据
 */
export const fetchMonthlyStats = async (): Promise<Array<{
  day: string;
  duration: number;
}>> => {
  try {
 ...
 * 获取热力图数据
 */
export const fetchHeatmapData = async (): Promise<Array<{
  date: string;
  duration: number;
}>> => {
  try {
 ...
 * 获取时间分布数据
 */
export const fetchTimeDistribution = async (): Promise<Array<{
  hour: number;
  duration: number;
}>> => {
  try {
...
export const fetchContentStats = async (): Promise<{
  dailyContentStats: Array<{ name: string; duration: number }>;
  weeklyContentStats: Array<{ name: string; duration: number }>;
  monthlyContentStats: Array<{ name: string; duration: number }>;
}> => {
  try {
    // 获取所有任务
    ...
    // 按内容分类统计任务时长
    const dailyContentMap = new Map<string, number>();
    const weeklyContentMap = new Map<string, number>();
    const monthlyContentMap = new Map<string, number>();
 ...
 * 获取所有统计数据
 */
export const fetchAllStatistics = async (): Promise<StatisticsData> => {
  try {
