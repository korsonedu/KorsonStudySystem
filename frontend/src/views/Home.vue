...
const getPlanId = (plan: any): number | string | undefined => {
  if (!plan) return undefined
  // 使用类型断言来避免TypeScript错误
  return plan.id || (plan as any)._id || (plan as any).planId
}
...
onMounted(() => {
  ...
  setTimeout(() => {
    ...
    // 添加重试机制
    const fetchWithRetry = async (fetchFn: Function, name: string, maxRetries = 3) => {
      let retries = 0
