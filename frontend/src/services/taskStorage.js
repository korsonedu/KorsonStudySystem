...
export class TaskStorageService {
    ...
    async getAllTasks() {
        try {
            ...
            // 获取本地任务
            const localTasks = storage.getItem(TASKS_STORAGE_KEY, []);
            ...
            for (const localTask of localTasks) {
                // 检查是否已存在于API任务中
                const exists = apiTasks.some((apiTask) => this.areTasksEqual(apiTask, localTask));
                if (!exists) {
                    mergedTasks.push(localTask);
                }
            }
            // 按开始时间排序（最新的在前面）
            return mergedTasks.sort((a, b) => new Date(b.start).getTime() - new Date(a.start).getTime());
        }
        catch (error) {
            console.error('Error getting all tasks:', error);
            // 如果出错，返回本地任务
            return storage.getItem(TASKS_STORAGE_KEY, []);
        }
    }
    /**
     * 添加新任务
    ...
    async addTask(task) {
        try {
            ...
            if (apiSuccess && apiTask) {
                ...
                // 获取现有任务并添加新任务
                const tasks = storage.getItem(TASKS_STORAGE_KEY, []);
     ...
     * 同步本地任务到API
     */
    async syncLocalTasksToAPI(localTasks, apiTasks) {
        // 获取需要同步的任务（本地有但API没有的）
        const tasksToSync = localTasks.filter(localTask => !apiTasks.some(apiTask => this.areTasksEqual(apiTask, localTask)));
        ...
        // 同步每个任务
        const syncResults = await Promise.allSettled(tasksToSync.map(task => tasksAPI.createTask(task)));
        // 统计结果
