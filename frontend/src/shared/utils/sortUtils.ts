 ...
 * @returns 计划ID
 */
export function getPlanId(plan: any): number | string | undefined {
 ...
 * @returns 排序后的计划数组
 */
export function sortPlansByCompletionAndDate(plans: any[]): any[] {
  // 如果没有计划，直接返回空数组
 ...
 * @returns 今日计划数组
 */
export function filterTodayPlans(plans: any[]): any[] {
  // 如果没有计划，直接返回空数组
  if (!plans || !Array.isArray(plans) || plans.length === 0) {
 ...
 * @returns 排序后的任务数组
 */
export function sortTasksByDate(tasks: any[], ascending: boolean = false): any[] {
 ...
 * @returns 排序后的任务数组
 */
export function sortTasksByDuration(tasks: any[], ascending: boolean = false): any[] {
  return [...tasks].sort((a, b) => {
 ...
 * @returns 排序后的任务数组
 */
export function sortTasksByName(tasks: any[], ascending: boolean = true): any[] {
  return [...tasks].sort((a, b) => {
    const nameA = a.name || '';
    const nameB = b.name || '';
 ...
 */
export function sortTasksByCompletion(tasks: any[]): any[] {
  return [...tasks].sort((a, b) => {
    return a.completed ? 1 : -1;
  });
}
 ...
 * @returns 分组后的任务对象
 */
export function groupTasksByType(tasks: any[]): Record<string, any[]> {
